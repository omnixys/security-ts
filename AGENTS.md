...
> Repository development instructions for **security** (Shared Package).
> Stack: ts-package. Generated as part of the Omnixys repository-policy rollout.

## Repository Overview

Omnixys security primitives for TypeScript (JWT, auth, hashing).

- Repository path: `packages/ts/security` (relative to the Omnixys root)
...
Security-critical; never weaken authentication/authorization to make tests pass.
